...
def color_content(image):
	...
	least = codes[index_min]
	colour = binascii.hexlify(bytearray(int(c) for c in peak)).decode('ascii')
	least_colour = binascii.hexlify(bytearray(int(c) for c in least)).decode('ascii')	
...
def derivation_fun(image,rate):
	...
	mask = cv2.inRange(img, derivation, s)
	total_derivation_org = cv2.bitwise_and(img,img, mask= mask)
	total_derivation = cv2.bilateralFilter(total_derivation_org,12,9,7)
	total_derivation = cv2.convertScaleAbs(total_derivation, 3, beta=0)
	#smoth_derivation = cv2.bilateralFilter(derivation,12,9,7)
	smoth_derivation = cv2.convertScaleAbs(derivation, 3, beta=0)

	laplace = cv2.convertScaleAbs(laplace, 1.2, beta=0)

	sumit = cv2.add(s, derivation)
	# -0.1 smooth space between colors
	# everything black here is one suspect
	s = cv2.addWeighted(img, 1.0, total_derivation, -0.1*rate, 1)
	s = cv2.addWeighted(s, 1.0, sumit, -0.1*rate, 1)
	#s = cv2.addWeighted(img, 1.0, laplace, -0.1, 0)
	#s = cv2.addWeighted(s, 1.0, smoth_derivation, -0.2, 0)
	return s
...
for filename in os.listdir(inFile):
    if filename.endswith(".jpg") or filename.endswith(".png") or filename.endswith(".JPG"): 
		...
		# Try to improve edges
		#
		if(detail_entropy>7.5):
			img = derivation_fun(img,0.5)
		else:
			img = derivation_fun(img,1)	
		#######################
